...
public final class TicketInfo implements SerializableRead, Externalizable {
 ...
 // JG July 2014 Ticket creator Host - for ticket print
    ...
    public void setTicketId(int iTicketId) {
        ...
        this.setProperty("pickup_date", this.printDate());
        this.setProperty("has_pickedup", "true");
        this.setProperty("pickupid", this.printPickupId());
    // refreshLines();
    }
...
// JG Aug 2014 - Add User info
        ...
        if (info == null) {
            ...
            name.append(this.printPickupId());
        } else {
    ...
    public java.util.Date getPickupDate() {
        try {
            return DateFormat.getInstance().parse(this.getProperty("pickup_date", "0"));
        }
        catch(Exception ex) {
    ...
    public void setPickupDate(String date) {
        this.setProperty("pickup_date", date);
    }
    
    public void hasPickedUp() {
        this.setProperty("has_pickedup", "true");
    }
...
// We need acces to the config file        
  ...
  // we have finished with m_config so unload it      
        ...
        if (m_iPickupId > 0) {
            String tmpTicketId=Integer.toString(m_iPickupId);
            if (receiptSize == null || (Integer.parseInt(receiptSize) <= tmpTicketId.length())){
                if (receiptPrefix != null){
                    tmpTicketId=receiptPrefix+tmpTicketId;
                } 
    ...
    public String printPickupDate() {
        return this.getProperty("pickup_date", "None");
    }
    
    public String printTagNumber() {
        return this.getProperty("tagnumber", "0");
    }
